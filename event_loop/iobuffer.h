 ...
 * Author: lap11894
 *
 * Created on January 7, 2019, 4:49 PM
 */

#ifndef ZRTC_IOBUFFER_H
#define ZRTC_IOBUFFER_H
...
public:
    ...
    static const size_t kCheapPrependSizeByte;
    static const size_t kInitialSizeByte;

    explicit IOBuffer(size_t initial_size = kInitialSizeByte,
					size_t reserved_prepend_size = kCheapPrependSizeByte)
	...
	explicit IOBuffer(const char *data, size_t size,
		...
		buffer_ = new char [capacity_];
		Append(data, size);
		switch (reserved_prepend_size_) {
			case 1:
				PrependInt8(capacity_);
				break;
			case 2:
				PrependInt16(capacity_);
				break;
			case 4:
				PrependInt32(capacity_);
				break;
			case 8:
				PrependInt64(capacity_);
				break;
			default:
				Prepend(static_cast<const void *>(&capacity_), sizeof(capacity_));
				break;
		}
